...
## Command Line Options:
    ...
    --post-de-signal-policy:
                                    ...
                                    skip: record skips when BigWigs are absent
                                    Default: auto
...
- Step 10 creates annotation-derived gene-body BED features.
- Step 11 counts reads over those gene bodies.
...
- Post-DE `analyze_peaks_de` still runs for gene-body mode and produces:
  ...
  - promoter-derived BED sets for motif analysis of DE genes

Current ChIP diffuse behavior:
- Step 10 derives fixed genomic bins from the BAM header chromosome sizes, filters them to standard chromosomes by default, excludes chrM by default, and subtracts the ENCODE blacklist when available.
- Step 11 counts reads over those bins.
- Pre-DE `analyze_peaks` produces bin metadata plus global signal summaries for all bins.
- Step 15 runs chromatin DE on bins with diffuse-specific filtering defaults.
