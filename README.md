...
- ATAC/ChIP post-DE GimmeMotifs runs are bounded by workflow defaults (`post_de_motif_max_sets`, `post_de_motif_max_peaks`, `post_de_motif_timeout_seconds`, `post_de_motif_threads`). The default prioritizes promoter/promoter-region sets, then distal/top-ranked sets, and records skipped, failed, or timed-out motif runs in `analyze_peaks_de/motifs/motif_runs.tsv` without failing the whole post-DE report.
