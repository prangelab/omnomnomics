...
The DE_calling folder contains RNA count tables and step-12 DESeq2 outputs. Step 12 now renders a reproducible run script plus a customization guide script, writes resolved metadata, and generates a DE results tree under `DE_calling/<de_out_dir>`.
...
12:	RNA only:	   Call DE genes (DESeq2; implemented)
...
## Command Line Options:
    ...
    -m <METADATA_TABLE>:    .txt file with columns of metadata for RNA-seq experiments.
                                    Metadata table describing all samples. First column should be `filename`.
                                    Step 12 derives `sample_id` internally and writes `metadata_derived.tsv`.
