...
rule bam_stats_experiment_summary:
    input:
        sample_stats=expand(
            f"{experiment_dir}/{master_config['output_folders'][master_config['stats_rule_num']-1]}/{{sample}}.sorted.dups_marked.filtered.bam.stats.txt"
            if config['THETYPE'] != "CHIP"
            else f"{experiment_dir}/{master_config['output_folders'][master_config['stats_rule_num']-1]}/{{sample}}.filtered.bam.stats.txt",
            sample=samples2,
        )
    output:
        summary_pdf=f"{experiment_dir}/{master_config['output_folders'][master_config['stats_rule_num']-1]}/{os.path.basename(config['EXPERIMENT_DIR'])}.alignment_qc_experiment_summary.pdf",
        summary_svg=f"{experiment_dir}/{master_config['output_folders'][master_config['stats_rule_num']-1]}/{os.path.basename(config['EXPERIMENT_DIR'])}.alignment_qc_experiment_summary.svg"
    params:
    ...
    resources:
        ...
        runtime=Runtime_Per_Rule['7']
    run:
        log_it(logfile, "Generating experiment-level alignment QC summary plots...", "ALIGNMENT QC SUMMARY")

        def load_sample_metric_table(tsv_paths):
            rows = []
        ...
        def draw_metric_panel(ax, rows, metric_specs, title, ylabel, log_scale=False):
            point_color = "#355070"
            summary_color = "#e56b6f"
            box_face_color = "#dbe7ee"
            box_edge_color = "#8aa1b1"
            valid_specs = []
            for label, stage, metric in metric_specs:
                samples_for_metric, values = metric_values(rows, stage, metric)
                valid_specs.append((label, samples_for_metric, values))
            ...
            for idx, (label, samples_for_metric, values) in enumerate(valid_specs):
                if not values:
            ...
            ax.set_xticks(range(len(valid_specs)))
            ax.set_xticklabels([label for label, _, _ in valid_specs], rotation=15, ha="right")

        rows = load_sample_metric_table(input.sample_stats)
        fig, axes = plt.subplots(2, 2, figsize=(14, 10))
        fig.patch.set_facecolor("white")
        fig.suptitle(f"{params.experiment_name} alignment QC experiment summary", fontsize=16, fontweight="bold")
        ...
        draw_metric_panel(
            ...
            rows,
            [
                ("Pre mapped reads", "pre_filter", "mapped_primary_reads"),
                ("Post mapped reads", "post_filter", "mapped_primary_reads"),
                ("Pre duplicate reads", "pre_filter", "duplicate_primary_reads"),
            ],
            "Read-level abundance",
            "Reads",
            log_scale=True,
        )
        ...
        draw_metric_panel(
            ...
            rows,
            [
                ("Pre proper pairs", "pre_filter", "properly_paired_templates"),
                ("Post proper pairs", "post_filter", "properly_paired_templates"),
                ("Pre discordant pairs", "pre_filter", "discordant_templates"),
            ] if params.paired else [
                ("Pre mapped reads", "pre_filter", "mapped_primary_reads"),
                ("Post mapped reads", "post_filter", "mapped_primary_reads"),
                ("Post duplicate reads", "post_filter", "duplicate_primary_reads"),
            ],
            "Pairing structure" if params.paired else "Post-filter abundance",
            "Pairs" if params.paired else "Reads",
            log_scale=True,
        )

        fig.tight_layout(rect=[0, 0.02, 1, 0.95])
        fig.savefig(output.summary_pdf)
        fig.savefig(output.summary_svg)
        plt.close(fig)
