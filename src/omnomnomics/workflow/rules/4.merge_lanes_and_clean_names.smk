            ...
            if input_bamfiles:
                ...
                for sample_key, lanes in sample_lane_dict.items():
                    ...
                    if num_lanes > 1:
                        ...
                        #create temporary files to ensure that the original files with lane info are still there at the end so that rule all is complete
                        for bamfile in bam_list:
                            tmp_bam_file = bamfile.replace(".bam", ".copy.bam")
                            tmp_files_list.append(tmp_bam_file)
                            shell(f"cp {bamfile} {tmp_bam_file}")
                        #merge files of same sample
                        shell(f"""
                            module load samtools && \
                            samtools merge -@ {threads} -o {inputfolder}/{myname}{ext} {' '.join(bam_list)}""")
                        #move original files back
                        for tmp_bam_file in tmp_files_list:
                            original_bam_file = tmp_bam_file.replace(".copy.bam", ".bam")
                            shell(f"mv {tmp_bam_file} {original_bam_file}")
