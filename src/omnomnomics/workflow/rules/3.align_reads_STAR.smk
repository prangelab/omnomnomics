...
rule run_star:
    ...
    params:
        ...
        paired=lambda wildcards: sample_is_paired(wildcards.sample3)
    threads:
        Threads_Per_Rule['3']
    resources:
    ...
    run:
        ...
        def run_star(genome_path, fastq1, fastq2, paired, threads, outputfolder, sample):
            ...
            try:
                ...
                local_final_stats = os.path.join(local_workdir, f"{sample}.STAR_stats.txt")

                if not os.path.exists(local_aligned_bam):
                    raise FileNotFoundError(f"Expected STAR BAM output for {sample} at {local_aligned_bam}")
                if not os.path.exists(local_star_stats):
                    raise FileNotFoundError(f"Expected STAR stats output for {sample} at {local_star_stats}")
        ...
        run_star(params.genome_path, input.trimmed_fastq1, input.trimmed_fastq2, params.paired, threads, params.outputfolder, wildcards.sample3)
