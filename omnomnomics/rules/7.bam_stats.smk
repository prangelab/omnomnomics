...
rule bam_stats:
    input:
        ...
        bai_BAM = f"{master_config['input_folders'][master_config['stats_rule_num']-1]}/{{sample}}.sorted.dups_marked.filtered.bam.bai" if config['THETYPE'] != "CHIP" else f"{master_config['input_folders'][master_config['stats_rule_num']-1]}/{{sample}}.filtered.bam.bai"
    output:
        f"{master_config['output_folders'][master_config['stats_rule_num']-1]}/{{sample}}.sorted.dups_marked.filtered.bam.stats.txt" if config['THETYPE'] != "CHIP" else f"{master_config['output_folders'][master_config['stats_rule_num']-1]}/{{sample}}.filtered.bam.stats.txt"
    params:
