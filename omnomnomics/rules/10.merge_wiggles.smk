...
def input_function(wildcards):
    input_folder = master_config['input_folders'][master_config['mergewig_rule_num']-1]
...
rule merge_wiggles:
    input:
        ...
        extra_file = expand(f"{master_config['input_folders'][master_config['mergewig_rule_num']-1]}/{{sample}}.extra_9.tmp", sample = samples2) if 9 in themode else []
        #expand(f"{master_config['input_folders'][master_config['mergewig_rule_num']-1]}/{{sample}}.extra_9.tmp", sample = samples2) if 9 in themode else None
    output:
        #theoutfolder=lambda wildcards: master_config['output_folders'][master_config['mergewig_rule_num']-1]
        #directory()
        f"{master_config['output_folders'][master_config['mergewig_rule_num']-1]}/extra_10.tmp"
    ...
    params:
        ...
        inputfolder=lambda wildcards: master_config['input_folders'][master_config['mergewig_rule_num']-1],
        outputfolder=lambda wildcards: master_config['output_folders'][master_config['mergewig_rule_num']-1]
    threads:
    ...
    benchmark:
        f"{master_config['output_folders'][master_config['mergewig_rule_num']-1]}/merge_wiggles_benchmark.tsv"
    run:
        ...
        def merge_wig(input_folder, output_folder, type_, col_field, separator, type_field, name_fields, col_table, appendix, genome, hub_mail, overlay):
            ...
            if type_ == "RNA":
                ...
                with open(os.path.join(f"{OMNOM_HOME}", "bin/color_data_for_hubs/poscols.hub")) as f: ###########change first part to OMNOM_HOME
                    poscols = f.read().splitlines()

                with open(os.path.join(f"{OMNOM_HOME}", "bin/color_data_for_hubs/negcols.hub")) as f: ##########change first part to OMNOM_HOME
