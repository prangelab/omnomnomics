...
def input_function(wilcards):
    ...
    if config['THETYPE'] != "CHIP":
        if config['THETYPE'] == "ATAC":
            input_files.append(f"{master_config['input_folders'][master_config['countreads_rule_num']-1][1]}/extra_11.tmp")
...
rule count_reads:
    ...
    output:
        ...
        f"{master_config['output_folders'][master_config['countreads_rule_num']-1]}/extra_12.tmp" #must specify this so that it always performs this rule if asked for, also vor CHIP.
    params:
        ...
        inputfolder1 = master_config['input_folders'][master_config['countreads_rule_num']-1][0],
        inputfolder2 = master_config['input_folders'][master_config['countreads_rule_num']-1][1],
    ...
    run:
        ...
        sanity_check_dir(logfile, params.inputfolder1,  master_config['input_file_types'][master_config['countreads_rule_num']-1][0])
