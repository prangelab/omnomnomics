...
rule call_DE:
    input:
        f"{master_config['input_folders'][master_config['de_rule_num']-1]}/{{sample}}.raw_read_quant.table.txt"
    output:
        f"{master_config['output_folders'][master_config['de_rule_num']-1]}/{{sample}}.results.zip"
    params:
    ...
    run:
        def calling_DE(logfile, thetype, inputfolder, outputfolder):
            if thetype == "RNA":
                ...
                sanity_check_dir(logfile, params.inputfolder,  master_config['input_file_types'][master_config['de_rule_num']-1])
                #....
            elif thetype == "ATAC":
