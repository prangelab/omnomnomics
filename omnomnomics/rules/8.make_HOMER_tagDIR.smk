...
rule create_homer_tagDir:
    input:
        filtered_BAM=f"{master_config['input_folders'][master_config['tagdir_rule_num']-1]}/{{sample}}.sorted.dups_marked.filtered.bam" if config['THETYPE'] != "CHIP" else f"{master_config['input_folders'][master_config['tagdir_rule_num']-1]}/{{sample}}.filtered.bam",
        bai_BAM = f"{master_config['input_folders'][master_config['tagdir_rule_num']-1]}/{{sample}}.sorted.dups_marked.filtered.bam.bai" if config['THETYPE'] != "CHIP" else f"{master_config['input_folders'][master_config['tagdir_rule_num']-1]}/{{sample}}.filtered.bam.bai"
    output:
        ...
        f"{master_config['output_folders'][master_config['tagdir_rule_num']-1]}/{{sample}}.extra_8.tmp"
    params:
    ...
    benchmark:
        f"{master_config['output_folders'][master_config['tagdir_rule_num']-1]}/{{sample}}_make_homer_tagdirs_benchmark.tsv"
    run:
